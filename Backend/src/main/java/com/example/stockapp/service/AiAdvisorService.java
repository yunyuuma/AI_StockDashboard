...
public class AiAdvisorService {
    ...
    public AiAdvisorResponse analyze(Long userId) {
        ...
        BigDecimal total = safe(pf.getTotalAsset());
        BigDecimal cash = safe(pf.getCash());
        BigDecimal stock = safe(pf.getStockValue());
        BigDecimal pl = safe(pf.getProfitLoss());
        BigDecimal dailyPl = safe(pf.getDailyProfitLoss());
        BigDecimal maxDDRate = safe(pf.getMaxDrawdownRate());

        BigDecimal cashRate = pct(cash, total);
        BigDecimal stockRate = pct(stock, total);

        portfolioAdvice.add(pl.compareTo(BigDecimal.ZERO) >= 0
                ? "総損益はプラスです。現在の運用成績は比較的良好です。"
                : "総損益はマイナスです。銘柄選定や売買タイミングの見直し余地があります。");
        portfolioAdvice.add(dailyPl.compareTo(BigDecimal.ZERO) >= 0
                ? "日次損益はプラスです。短期的には資産が増加しています。"
                : "日次損益はマイナスです。直近の値動きには注意が必要です。");
        ...
        if (maxDDRate.compareTo(BigDecimal.valueOf(10)) >= 0) {
        ...
        List<SectorAllocationResponse> sectors = pf.getSectorAllocations();
        if (sectors != null && !sectors.isEmpty()) {
            SectorAllocationResponse top = sectors.get(0);
            if (safe(top.getRate()).compareTo(BigDecimal.valueOf(50)) >= 0) {
                portfolioAdvice.add(top.getSector() + " の比率が高く、セクター集中リスクがあります。");
        ...
        if (tradeCount == 0) tradingAdvice.add("売買履歴がまだありません。まずは少額の疑似売買で履歴を作ると分析精度が上がります。");
        else if (tradeCount < 5) tradingAdvice.add("売買履歴が少なめです。もう少し取引データが増えると傾向分析しやすくなります。");
        else tradingAdvice.add("売買履歴が蓄積されています。今後は勝率・損切り頻度・利益確定タイミングの分析が可能です。");

        warnings.add("このAI分析は投資助言ではなく、疑似売買データに基づく学習用コメントです。");

        String risk = decideRisk(cashRate, maxDDRate, sectors);
        ...
        return new AiAdvisorResponse(risk, summary, portfolioAdvice, tradingAdvice, warnings);
    }

    private String decideRisk(BigDecimal cashRate, BigDecimal ddRate, List<SectorAllocationResponse> sectors) {
        boolean sectorRisk = sectors != null && !sectors.isEmpty()
                && safe(sectors.get(0).getRate()).compareTo(BigDecimal.valueOf(50)) >= 0;
        if (cashRate.compareTo(BigDecimal.valueOf(15)) < 0 || ddRate.compareTo(BigDecimal.valueOf(10)) >= 0 || sectorRisk) return "HIGH";
        if (cashRate.compareTo(BigDecimal.valueOf(30)) < 0 || ddRate.compareTo(BigDecimal.valueOf(5)) >= 0) return "MIDDLE";
        return "LOW";
    }
    ...
    private BigDecimal pct(BigDecimal num, BigDecimal den) {
        ...
        return num.multiply(BigDecimal.valueOf(100)).divide(den, 2, java.math.RoundingMode.HALF_UP);
    }
}
